...
namespace HrSite.Tool
{
    public class WebConfigManager : IDisposable
    {
        ...
        public void SetAppSettings(Dictionary<string, string> dic)
        {
            var appSetting = (AppSettingsSection)_config.GetSection("appSettings");
            foreach (KeyValuePair<string, string> kvp in dic)
            {
                if (appSetting.Settings[kvp.Key] == null)
                {
                    appSetting.Settings.Add(kvp.Key, kvp.Value);
                }
                else
                {
                    appSetting.Settings[kvp.Key].Value = kvp.Value;
                }
            }
            Save();
        }
        ...
        /// <summary>
        /// 获取webconfig配置文件中的AppSettings节点下的内容
        /// </summary>
